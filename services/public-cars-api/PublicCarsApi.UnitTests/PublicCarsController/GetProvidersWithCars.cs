...
{
    public class GetProvidersWithCars
    {
        [Fact]
        public async void GetPublicCars()
        {
            // Arrange
            var car = new ExternalCar()
            {
                Id = "123",
                Lat = 53,
                Lon = 50,
            ...
            // Act
            var result = await controller.GetProvidersWithCarsAsync(new FilterOptions());

            // Assert
            var resultProvider = result.First();
            Assert.True(resultProvider.Id == "provider");
            Assert.True(resultProvider.Cars.Count() == 1);
            Assert.True(resultProvider.Cars.First().Id == "123");
        }
    }
}
